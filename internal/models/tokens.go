...
type Token struct {
	PlainText string    `json:"token"` // actual token itself
	UserID    int64     `json:"-"`
	Hash      []byte    `json:"-"`
...
func GenerateToken(userID int, ttl time.Duration, scope string) (*Token, error) { //ttl stands for Time to life, duration\
	token := &Token{
		UserID: int64(userID),
...
func (m DBModel) GetUserForToken(token string) (*User, error) {
	...
	query := `
		...
		where
		ADDDATE(t.created_at , INTERVAL 24 HOUR) > NOW() AND
