...
// Status is teh type for order statuses
type Status struct {
	ID        int       `json:"id"`
	Name      string    `json:"name"`
	CreatedAt time.Time `json:"-"`
...
// TransactionStatus is teh type for transaction statuses
...
// Transaction is teh type for transactions
type Transaction struct {
	...
	TransactionStatusID int       `jason:"transaction_status_id"`
	CreatedAt           time.Time `json:"-"`
	UpdatedAt           time.Time `json:"-"`
}
...
// Customer is pthe type for customers
...
func (m *DBModel) InsertTransaction(txn Transaction) (int, error) {
	//int is an ID of newly inserted transacrion
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
//InsertOrder inserts a new customer, and returns its id
