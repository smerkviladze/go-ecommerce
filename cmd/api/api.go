...
type config struct {
	...
	env  string
	db   struct {
		dsn string //dns = data source name
	}
	stripe struct {
		secret string
		key    string
	...
	secretkey string //the key that we use to sign our URLs
	frontend  string //the address for our front end
...
func main() {
	...
	flag.StringVar(&cfg.smtp.username, "smtpuser", "d1fe77352e9a80", "smtp user")
	flag.StringVar(&cfg.smtp.password, "smtppass", "7796886bbac61c", "smtp password")
	...
	infoLog := log.New(os.Stdout, "INFO\t", log.Ldate|log.Ltime) // \t = Tab
