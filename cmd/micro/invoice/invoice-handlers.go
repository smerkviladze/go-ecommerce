...
//Order describes the json payload received by this microservice, for generating a simple invoice
type Order struct {
	ID        int       `json:"id"`
...
func (app *application) createInvoicePDF(order Order) error {
	...
	temp := importer.ImportPage(pdf, "./pdf-templates/invoice.pdf", 1, "/MediaBox")

	pdf.AddPage()
	importer.UseImportedTemplate(pdf, temp, 0, 0, 215.9, 0)
